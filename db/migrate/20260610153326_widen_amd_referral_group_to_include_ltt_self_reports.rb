class WidenAmdReferralGroupToIncludeLttSelfReports < ActiveRecord::Migration[8.1]
  # AMD sponsored the Linus Tech Tips (LTT) placement, so every signup sourced
  # from LTT belongs in the AMD rollup. The prior amd_referral_group flag was
  # derived purely from primary_referral_source = 'amd', i.e. the last-touch
  # signal (a /ref URL or a first_user_ref of 'linus tech tips'). That misses
  # people who saw the LTT video but entered through a different door (program
  # page, hc-email, GPU raffle code, GitHub edu, ...) and only self-report
  # "Linus Tech Tips" in the survey. Those rows carry the value in raw_user_refs
  # (the de-duplicated array of every user_ref the person ever gave) but not as
  # their first_user_ref, so they leaked out of AMD (~111 of ~294 LTT signups).
  #
  # The fix widens ONLY the amd_referral_group grouping flag: a row is 'AMD' if
  # the existing last-touch condition holds OR raw_user_refs contains
  # "Linus Tech Tips" (case-insensitive). primary_referral_source,
  # actual_referral_source, source_group, and the assumed-source distribution
  # are deliberately left untouched, so AMD can still be broken down by entry
  # path. Validated against the prod warehouse mirror: LTT-outside-AMD leak goes
  # 294 -> 0, AMD total rises by exactly the 111 previously-leaking LTT rows,
  # and no row is double-flagged or un-flagged.
  #
  # Materialized views can't be CREATE OR REPLACE-d, and the prior definition
  # (20260608175849) already ran in production, so we drop and recreate. The
  # only change from that migration is the amd_referral_group expression; the
  # banned-user anti-join and the Linus-Tech-Tips => AMD direct mapping are
  # preserved. See 20260608132316 for the full rationale on why this matview
  # lives outside schema.rb.
  def up
    recreate_view!(amd_referral_group_case: <<~SQL.strip)
  ...
  def recreate_view!(amd_referral_group_case:)
    safety_assured do
      ...
      execute <<~SQL
        COMMENT ON MATERIALIZED VIEW materialized_all_signups IS
          ...
          'amd_referral_group additionally rolls up ANY person who self-reports '
          '''Linus Tech Tips'' in raw_user_refs (case-insensitive) into AMD, even '
          'when they entered through a different last-touch channel -- so the AMD '
          'group captures the full LTT activation while primary_referral_source / '
          'source_group still preserve the entry-path breakdown. '
