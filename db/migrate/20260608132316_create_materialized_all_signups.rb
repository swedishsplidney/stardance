class CreateMaterializedAllSignups < ActiveRecord::Migration[8.1]
  # Reporting matview, deliberately NOT in schema.rb (vanilla schema dumper
  # ignores materialized views). It is created here directly and only exists in
  # databases that have run this migration (i.e. production). It is queried by
  # Blazer / AI agents / external reporting, not by Rails app code or tests, so
  # schema-load environments (CI/test/fresh dev) intentionally do without it.
  #
  # Definition mirrors the Stardance signup-attribution query: one row per
  # normalized email, deduped across `users` and `rsvps`, with geocoding
  # (native + fallback) and referral attribution (direct + deterministic
  # assumed-source). The unique index on `email` is what makes
  # REFRESH MATERIALIZED VIEW CONCURRENTLY possible (see
  # RefreshMaterializedAllSignupsJob, run every 2 minutes via config/recurring.yml).
  def up
    safety_assured do
      execute <<~SQL
      ...
      execute <<~SQL
        COMMENT ON MATERIALIZED VIEW materialized_all_signups IS
          ...
          'actual_referral_source_from to distinguish observed vs assumed). '
