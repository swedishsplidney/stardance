class AttributeLttSelfReportsToAmdPrimarySource < ActiveRecord::Migration[8.1]
  # Promote Linus Tech Tips (LTT) attribution from a grouping-only flag to the
  # canonical primary_referral_source. AMD sponsored the LTT placement, so every
  # person who self-reports "Linus Tech Tips" anywhere in raw_user_refs is now
  # credited to AMD at the source level: direct_referral_source = 'amd' takes top
  # priority, ahead of the /ref last-touch channel (ambassador / raffle / gh-edu).
  # That flows into primary_referral_source, source_group, and amd_referral_group.
  #
  # This supersedes 20260610153326, which only widened the amd_referral_group flag
  # (keeping primary_referral_source at the last-touch channel). Because every LTT
  # row now resolves to primary_referral_source='amd', the raw_user_refs text match
  # in amd_referral_group is redundant and is reverted to the simple
  # primary_referral_source='amd' rule. A primary_referral_methodology branch is
  # added so these rows explain that LTT self-report overrode the last-touch
  # channel. actual_referral_source / known_referral_source are left untouched, so
  # the raw observed last-touch value is still visible per row.
  #
  # Side effect (intended): LTT signups that previously counted toward
  # gpu_raffle / stardance_ambassador / github in the known-source distribution
  # now count toward amd, which shifts the deterministic assumed-source assignment
  # for no-signal rows toward amd accordingly.
  #
  # Materialized views can't be CREATE OR REPLACE-d and 20260610153326 already ran
  # in production, so we drop and recreate. The banned-user anti-join is preserved.
  # See 20260608132316 for why this matview lives outside schema.rb.
  def up
    recreate_view!(
      direct_referral_source_case: <<~SQL.strip,
        case when lower(coalesce(raw_user_refs::text,'')) like '%linus tech tips%' then 'amd'
             when lower(coalesce(first_ref,'')) like 'a-%' then 'stardance_ambassador'
      ...
      SQL
      primary_referral_methodology_case: <<~SQL.strip,
        case when lower(coalesce(raw_user_refs::text,'')) like '%linus tech tips%' then 'AMD: self-reported Linus Tech Tips in raw_user_refs; attributed to AMD over any last-touch channel. actual_referral_source still shows the raw observed value.'
             when direct_referral_source is not null and first_ref is not null then 'Direct: /ref URL suffix present; ref prioritized over user_ref.'
      ...
      SQL
      amd_referral_group_case: <<~SQL.strip,
  ...
  def down
    recreate_view!(
      ...
      SQL
      amd_referral_group_case: <<~SQL.strip,
        case when primary_referral_source='amd'
                  or lower(raw_user_refs::text) like '%linus tech tips%'
             then 'AMD' else 'Other' end as amd_referral_group,
      SQL
    )
  end
  ...
  def recreate_view!(direct_referral_source_case:, primary_referral_methodology_case:, amd_referral_group_case:)
    safety_assured do
      ...
      select email, first_seen_at_utc, first_seen_day_et, seen_in_rsvps, seen_in_users, ip_address,
        ...
        #{primary_referral_methodology_case},
      ...
      execute <<~SQL
        COMMENT ON MATERIALIZED VIEW materialized_all_signups IS
          ...
          'actual_referral_source_from to distinguish observed vs assumed). Anyone '
          'who self-reports ''Linus Tech Tips'' anywhere in raw_user_refs is '
          'credited to AMD (amd) as their primary_referral_source -- this overrides '
          'the /ref last-touch channel, since AMD sponsored that placement -- so it '
          'flows into source_group and amd_referral_group too. actual_referral_source '
          '/ known_referral_source still show the raw observed last-touch value. '
