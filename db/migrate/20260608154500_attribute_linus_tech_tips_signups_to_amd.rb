class AttributeLinusTechTipsSignupsToAmd < ActiveRecord::Migration[8.1]
  # AMD sponsored the Linus Tech Tips placement, so sign-ups whose only
  # attribution signal is the post-sign-up survey value "Linus Tech Tips"
  # (user_ref = 'linus tech tips'; there is no /ref URL variant in the data)
  # should be credited to AMD. This mirrors the existing 'gh-edu' => 'github'
  # mapping: it resolves a raw referral value to a canonical source at the
  # direct_referral_source layer, so it flows through to primary_referral_source,
  # source_group, amd_referral_group, and the assumed-source distribution while
  # leaving actual_referral_source / known_referral_source showing the observed
  # raw value ("Linus Tech Tips").
  #
  # Materialized views can't be CREATE OR REPLACE-d, and the original definition
  # (20260608132316) already ran in production, so we drop and recreate. See that
  # migration for the full rationale on why this matview lives outside schema.rb.
  def up
    recreate_view!(direct_referral_source_case: <<~SQL.strip)
  ...
  def recreate_view!(direct_referral_source_case:)
    safety_assured do
